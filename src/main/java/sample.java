...
public class sample extends QuanticAPIBase {


    public static void main(String[] args) {

        JsonObject kity = (JsonObject) JsonParser.parseString("{\n" +
                "\t\"name\": \"akash the great#unique-4##curr-ddMMyyyy#\",\n" +
                "\t\"roll no\": \"484\",\n" +
                "\t\"time\": \"monday#curr-ddMM#\"\n" +
                "}");
        JsonObject lota = (JsonObject) JsonParser.parseString("{\n" +
                "\t\"name\": \"Pawan the great #unique-2#\",\n" +
                "\t\"roll no\": \"48#epoch-3#4\",\n" +
                "\t\"time\": \"monday\"\n" +
                "}");
        JsonArray soni = new JsonArray();

        // JsonObject lota=new JsonObject();
        soni.add(kity);
        soni.add(lota);

        //  JsonObject kity=new JsonObject();
        //  kity.addProperty("phalu#unique-10#","#CURR-ddMMyyyyhhmmss#tingu");
        //    JsonParser.parseString(String.valueOf(soni));

        //   soni.add(lota);
        // soni.add(kity);
        JsonElement mo = papu(soni);
        System.out.println(mo);


    }

    public static JsonElement papu(JsonElement job) {
        ...
        JsonElement pope = (JsonElement) JsonParser.parseString(var2);
